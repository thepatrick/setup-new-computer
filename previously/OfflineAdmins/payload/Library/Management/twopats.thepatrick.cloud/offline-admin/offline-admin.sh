...
log() {
  LOGFILE="/var/log/offline-admin.log"
  echo $(date) "${@}" 2>&1 > $LOGFILE
  logger -t offline-admin "${@}"
}
...
log "Local Administrator Check (Host Name: ${HOSTNAME})"

# Find out what the existing Allowed Administrator group is
readonly CombinedADGroups=$( dsconfigad -show | grep "Allowed admin groups" | awk '{print substr ($0, index($0, $5)) }' )
...
for i in "${BIGARR[@]}"; do
	...
	log "Domain: ${NetBIOSNameArray[${#NetBIOSNameArray[@]}-1]}, Group: ${CombinedADGroupArray[${#CombinedADGroupArray[@]}-1]}"
done
...
do
	...
	for (( c=0 ; c<=${#NetBIOSNameArray[@]}-1; c++ )); do
		...
		if [[ "$ADGroups" =~ "${NetBIOSNameArray[$c]}" ]]; then
			...
			if [[ "$ADGroups" =~ "(${CombinedADGroupArray[$c]})" ]]; then
				log "[$localUser] is member of ${CombinedADGroupArray[$c]}"
				setLocalAdmin="Yes"
			fi
			shopt -u nocasematch
		else
	...
	if [[ "$setLocalAdmin" = "NotAD" ]]; then
		log "[$localUser] not an AD user"
	elif [[ "$setLocalAdmin" = "Yes" ]]; then
		log "[$localUser] should be an admin user"
		/usr/sbin/dseditgroup -o edit -a $localUser -t user admin
	else
		log "[$localUser] should be a standard user"
